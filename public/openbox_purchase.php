...
// Calculate open box price (10% off)
$openBoxPrice = round($product['price'] * 0.90, 2);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST['disclaimer'])) {
        ...
        if ($insertStmt->execute([$_SESSION['member_id'], $productId, $openBoxPrice, $address, $status])) {
            
            // Compose the confirmation email
            $messageBody  = "Dear $name,\n\n";
            $messageBody .= "Thank you for purchasing the open box version of " . $product['name'] . ".\n";
            ...
            $messageBody .= "Open Box Price (10% off): $" . number_format($openBoxPrice, 2) . "\n";
            $messageBody .= "Shipping Address: $address\n\n";
            $messageBody .= "Please note: Open box items are subject to limited stock, cannot be returned, and all sales are final.\n\n";
            $messageBody .= "Thank you for your purchase,\nGPU Paradise";
            
            // Retrieve user's email from DB lookup)
            ...
            if (empty($errorMessage)) {
                $subject = "Your Open Box Purchase Confirmation";
...
<head>
    <meta charset="UTF-8">
    <title>Buy Open Box: <?php echo htmlspecialchars($product['name']); ?></title>
        ...
        <div class="purchase-form-container">
            <?php if ($purchaseSuccess): ?>
                <p class="confirmation">
                    Thank you for purchasing the open box version of <strong><?php echo htmlspecialchars($product['name']); ?></strong>! We’ve sent a confirmation email to you.
                </p>
            <?php else: ?>
                <?php if (!empty($errorMessage)): ?>
                    <p class="error"><?php echo htmlspecialchars($errorMessage); ?></p>
                <?php endif; ?>
                <h2>Buy Open Box: <?php echo htmlspecialchars($product['name']); ?></h2>
                <p><strong>Price:</strong> $<?php echo number_format($openBoxPrice, 2); ?> (10% off)</p>
                <form method="POST" class="purchase-form">
                    ...
                    <div class="disclaimer">
                        ...
                        <label for="disclaimer">
                            I understand that open box products are subject to limited stock, cannot be returned, and all sales are final.
                        </label>
                    </div>
